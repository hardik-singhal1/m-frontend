...
function OrgSidebarList({ router, sidebarConfig }) {
    ...
    useEffect(async () => {
        if (userData && Object.keys(checkSideBarValue).length!==0 && sideBarResponse.length===0) {
            try {
    ...
    useEffect(()=>{
        if(router.isReady){
            let temp = router.asPath.split(`/${router.query.organization_name}/`)[1]
            if(temp===undefined){
                setPage("home")
                setChildPage("")
            }
...
OrgSidebarList.propTypes = {
    router: PropTypes.object.isRequired
};

export default React.memo(withRouter(OrgSidebarList));
